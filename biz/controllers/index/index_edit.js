...
const indexEditService = require('../../services/index_edit/index_edit');

exports.index = {
    path: '/index',
    method: 'get',
    handler: async (ctx) => {
        let originData = await indexEditService.getOriginData();
        let mainContent = await indexEditService.getMainContent();
        let siderList = await listTransform(originData);
        let data = {
            msg: 'HELLO WORLD',
            API: {
                getList: function () {
